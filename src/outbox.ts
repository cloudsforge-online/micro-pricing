 ...
 * EMPTY, it has no emit site yet and no topic of its own in the registry. Nothing is lost today;
 * the relay is simply wrong in advance, and it would have been discovered by the first consumer
 * that never heard from it.
 *
...
export function buildEnvelope(row: OutboxRow): EventEnvelope {
  return {
    ...
    // `system` is the contract's own value for "no principal did this" — a scheduled quote refresh, which
