...
> at [`src/pricing/`](https://github.com/cloudsforge-online/micro-agora/tree/main/src/pricing).
...
> tree to keep it that way** (`src/rates.ts`, `src/nofloat.test.ts`). Every decimal that
...
route of its own (`src/quotes.ts`). It holds no balances, moves no money and settles
...
**1. The quotes lived in a `Map`, and a `Map` is per-process** (`src/migrations.ts`). With
...
**2. Every quote went through a double, twice** (`src/rates.ts`). The old conversion was
...
`parseScaled` reads the decimal digits directly (`src/rates.ts`): `"0.512345"` produces
exactly `512345n` and `"64231.5"` produces exactly `64231500000n`. `formatScaled` renders back
without a float in either direction (`src/rates.ts`). `RATE_DECIMALS` is derived from
...
can pin down rather than a claim (`src/rates.ts`).

1. **Fetch all four sources with `Promise.allSettled`** (`src/oracle.ts`). One exchange being
...
2. **Parse each quote.** A malformed field costs that source its vote on that asset and nothing
   ...
   quietly reduce every round to three sources for ever (`src/oracle.ts`).
...
4. **Reject on divergence.** `divergenceBps` is `(high − low) × 10000 / low`, measured against the
   **lowest** so the number answers the question an operator actually asks — how far apart are
   these — without depending on the statistic the round is about to compute (`src/rates.ts`).
   ...
   (`src/rates.ts`). Taking the median anyway would launder a bad print into a rate that
   money then settles against, and a median of four is not robust to two wrong sources.
5. **Take the median, not the mean** (`src/rates.ts`). A mean is moved by a single bad
...
`refreshRound` **never throws for a price reason** (`src/oracle.ts`). A round in which every
...
this replaces, which got them right (`src/sources.ts`).
...
The transport is injected (`src/sources.ts`, `src/sources.ts`), so **no test in this
...
(`src/rates.ts`): a second list is a list that drifts, and the drift here would be an asset
...
would turn a fail-closed oracle into a fail-to-whatever-was-typed oracle (`src/rates.ts`,
`src/server.ts`).

Three refusals sit around that route:

* **A JSON number is refused**; `usd` must be a decimal string (`src/server.ts`).
  `{"usd": 0.1}` is already not 0.1 by the time this code sees it.
* **Extra precision from an operator is refused, not truncated** (`src/rates.ts`). A source
  ...
  believes they set.
* **An administered quote never goes stale** (`src/quotes.ts`, `src/quotes.ts`). It is
...
Read out of `src/server.ts`. Every decimal in every response is a string, including the
scaled integers, and every response carries `rateScale` so a consumer never has to assume the scale
it is doing BigInt arithmetic at (`src/quotes.ts`).

| Method | Path | Who | What it does |
| --- | --- | --- | --- |
| `GET` | `/livez` | public | static (`src/server.ts`) |
| `GET` | `/readyz` | public | 503 removes the replica from the balancer; a **soft** probe failure leaves the report degraded but still ready (`src/server.ts`) |
| `GET` | `/metrics` | public | Prometheus text; a gauge refresh that fails is warned, not fatal — failing the scrape would lose every other metric and blind the dashboard at the moment it is needed (`src/server.ts`) |
| `GET` | `/rates` | **public** | the whole board, plus `spreadBps` (`src/server.ts`) |
| `GET` | `/rates/:asset` | **public** | one rate; asset is upper-cased so `/rates/btc` works, and an unquoted asset is a 404 (`src/server.ts`, `src/server.ts`) |
| `PUT` | `/admin/prices/:asset` | `role:admin` **or** scope `pricing:admin` | set an administered price; returns the price and the rate it produces (`src/server.ts`) |
| `GET` | `/admin/prices` | same authority | every administered price with who set it (`src/server.ts`) |
| `GET` | `/history/:asset` | `pricing:read` **or** `role:admin` | newest first; `?limit` 1–1000, default 100 (`src/server.ts`, `src/server.ts`) |

**The rate board is public on purpose** (`src/server.ts`). It is public market data and it is
...
first and must never have the second (`src/server.ts`).

Two response choices are deliberate and easy to get wrong:

* **An unusable asset is listed, not omitted** (`src/server.ts`). Omitting it makes a client
  that iterates the board silently forget the asset exists, which is how a deposit page loses a coin.
* **`GET /rates/:asset` answers 200 even when the rate is unusable** (`src/server.ts`). The
...
**A bad token is 401; a verifier that could not reach the JWKS is 503** (`src/server.ts`).
...
(`src/quotes.ts`). Unusable for: no record, no quote yet, a non-positive stored value, no
...
(`src/rates.ts`): sell is `usd × (10000 − bps) / 10000`, buy is `usd × 10000 / (10000 − bps)`.
...
estate** (`src/jobs.ts`). The oracle it replaces runs `setInterval(tick, …)` guarded by nothing
...
| --- | --- | --- | --- |
| `outbox.relay` | `stream` | 1 s | the outbox stream — two relays deliver one batch twice (`src/jobs.ts`) |
...
claiming the refresh while this one is still finishing it (`src/jobs.ts`).
...
moment later and the schedule would stop (`src/jobs.ts`). **A dead-lettered recurring job is
...
(`src/migrations.ts`). A released migration is immutable: `@cloudsforge/db` checksums each one
...
| `inbox` | `(topic, event_id)` primary key; the key **is** the dedupe (`src/migrations.ts`) |
| `jobs` | `@cloudsforge/jobs`, imported verbatim rather than hand-copied (`src/migrations.ts`) |

The constraints and column choices that carry meaning:

| Constraint / choice | Refuses or prevents | Why here |
| --- | --- | --- |
| `usd_scaled numeric(78,0)`, **never `double precision`** | a rate that is not the rate that was written | Postgres' `float8` has 53 bits of mantissa, so a fixed-point rate stored as a float and read back is not necessarily the same number — which is the whole failure this service exists to avoid. It is read back as a string and converted with BigInt (`src/migrations.ts`, `src/quotes.ts`) |
| `check (usd_scaled > 0)` on all three tables | a zero or negative price, from **any** writer | including psql; a zero price is a conversion that credits nothing (`src/migrations.ts`) |
| `check (source in ('market','administered'))` | a third source kind appearing by accident | the distinction is carried to the client, because a conversion settled against an administered price was priced by a person and not by a market (`src/migrations.ts`) |
| `quoted_at` separate from `updated_at` | a failed round making a stale price look fresh | staleness is about the age of the **observation**; a failed round touches `updated_at` only (`src/migrations.ts`, `src/quotes.ts`) |
| the quote and its failure in **one row** | a rate board that cannot say why an asset is unusable | the old oracle kept them in two Maps that could disagree, so a coin whose round failed kept serving the previous median with no indication the last refresh was rejected (`src/migrations.ts`) |
| `outbox_unpublished_idx`, partial on `published_at is null` | an index the size of history | it stays the size of the backlog (`src/migrations.ts`) |
| `outbox_deliveries` PK `(event_id, subscription_id)` | one failing subscriber blocking or re-fanning the rest | with one flag on the outbox row, a single broken subscriber either blocks every other subscriber or causes redelivery to all of them on each retry (`src/migrations.ts`) |
| `price_history.id` is an identity `bigint`, not a v4 uuid | random insert positions in an append-only table | history is read in time order per asset, and a random key puts every insert in a different B-tree leaf (`src/migrations.ts`) |

An accepted round writes the quote **and** its history row in one transaction, because the history
is the evidence for the quote (`src/quotes.ts`). A quote with no history behind it is a rate
...
(`src/quotes.ts`), and there is no cache to fall out of step — the next `GET /rates` on any
replica reads the row.

**A failed round does not erase the last good quote**, it records why it failed
(`src/quotes.ts`). Deleting the quote would turn a transient exchange outage into an
...
between a configuration error and the report of it (`src/env.ts`). Non-integers are refused
rather than coerced.

| Variable | Default | If it is wrong |
| --- | --- | --- |
| `PRICING_DATABASE_URL` | — | refuses to start (`src/env.ts`) |
| `IDENTITY_JWKS_URL`, `IDENTITY_ISSUER` | — | refuses to start (`src/env.ts`) |
| `OUTBOX_SIGNING_SECRET` | — | refuses to start; must be long enough and must not be a known placeholder (`src/env.ts`) |
| `PRICING_MIN_SOURCES` | `2` (1–4) | **2 is the floor**: the smallest number at which the median is not one exchange's opinion. 1 is permitted only so a single-source development environment is possible, and the divergence check is inert at that setting because there is nothing to diverge from (`src/env.ts`) |
| `PRICING_MAX_DIVERGENCE_BPS` | `200` (1–10 000) | too high and a bad print is laundered into a rate; too low and every round is rejected (`src/env.ts`) |
| `PRICING_MAX_AGE_SECONDS` | `300` (5–86 400) | past this a market rate is unusable and says why. A stale quote is not a discount, it is an unknown price (`src/env.ts`) |
| `PRICING_CONVERSION_SPREAD_BPS` | `100` (0–**2 000**) | 10 000 would divide by zero on the buy leg; the bound is set far below that anyway, because 2 000 bps is a 20% spread and no honest platform charges it (`src/env.ts`) |
| `PRICING_SOURCE_TIMEOUT_MS` | `4000` (100–60 000) | a slow exchange holding the round open (`src/env.ts`) |
| `PRICING_REFRESH_SECONDS` | `60` (5–3 600) | how often the leased refresh runs — once for the estate, not once per replica (`src/env.ts`) |
| `PRICING_DATABASE_POOL_MAX` | `10` (1–100) | a pool larger than the database's budget divided by the replica count exhausts Postgres for everything else (`src/env.ts`) |
| `PORT` | `4000` | (`src/env.ts`) |
| `LOG_LEVEL` | `info` | outside debug/info/warn/error it refuses to start (`src/env.ts`) |
| `INSTANCE_ID` | hostname | names this replica in `jobs.locked_by` (`src/env.ts`) |
| `CLOUDSFORGE_TAG` | `dev` | reported on every log line (`src/env.ts`) |
...
between a red build and an emptied environment (`src/testsupport.ts`).
...
* **This service publishes no events, but ships the machinery to.** Migration 2 creates `outbox`,
  ...
  `OUTBOX_SIGNING_SECRET` is required at boot (`src/env.ts`) — yet `withOutbox`
  (`src/outbox.ts`) has no caller anywhere in `src/`, so the relay sweeps an empty table for
...
* **`recordAccepted` converts `divergenceBps` from BigInt to a JS number**
  (`src/quotes.ts`), because `divergence_bps` is an `integer` column. It is the only place a
  ...
  (`src/nofloat.test.ts`). So the guard against this line drifting into arithmetic is the
...
* **CoinGecko quotes as a JSON number**, so its value has been through a double before this code
  can see it (`src/sources.ts`). It is one of four inputs to a median, which is the
